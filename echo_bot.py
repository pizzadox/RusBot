...
# Подключаем модуль обработки ( не факт что тут, так как будeт скорее всего отдельным
# файлом расписано для каждого вида профиля и типа конструкии
...
class BotState:
    ...
    width = 0
    height = 0  # перенести в стуктура данных конкретного пользователя
...
def start(message):
    if message.from_user.id in bStates.keys():  # Проверяем есть ли пользователь в БД
        ...
        conn.close()
        print('заебок')
    elif message.from_user.id not in bStates.keys():
        bStates[message.from_user.id] = BotState()
        bot.send_message(message.from_user.id, "Ты у нас новенький, как тебя зовут ?")
        if message.text == 'вася':
            user_name = 'вася'
            conn = sqlite3.connect("orders.db")
            c = conn.cursor()
            c.execute("INSERT INTO users (telegram_id, name) VALUES (message.from_user.id, user_name)")
            conn.commit()
            print(c.fetchone())
            conn.close()
            print('хуй там')
...
def get_text_messages(message):
    ...
    if message.text == '👋 Поздороваться':
        ...
        bot.send_message(message.from_user.id,
                         'Вы уверены что понимаете что вам нужно ?\n \nПолный текст можно прочитать по ' + '[ссылке](https://oknarus.com/tpost/mclj26nlh1-zaklinilo-okno-chto-delat)',
                         parse_mode='Markdown')

    elif message.text == 'Нужна Дверь':
        bot.send_message(message.from_user.id,
                         'Прочитать правила сайта вы можете по ' + '[ссылке](https://oknarus.com/tpost/etcd6k6t11-okna-i-pozharnaya-bezopasnost)',
                         parse_mode='Markdown')

    elif message.text == 'Советы':
        bot.send_message(message.from_user.id, 'Подробно про советы по ' + '[ссылке](https://oknarus.com/#services)',
                         parse_mode='Markdown')
    # выбор окна
        ...
        for r in oknoConstr:
            ...
            description = r[1]
            image = r[2]
            markup.add(types.InlineKeyboardButton(text=r[0],
                                                  callback_data='{"user_id": %d,' % message.from_user.id + '"okno": %d}' %
                                                                r[3]))
            print(r[1])
...
def callback_inline(call):
    ...
    if call.data:  # //проверяем есть ли данные если да, далаем с ними что-то.
        ...
        if cb['user_id'] in bStates.keys():
            ...
            print(constr_id)
            ans = "Вы выбрали конструкцию ID %d" % constr_id
